...
| `Assets/Engineering/Scripts/Mono/Actors/CustomerQueue/` | `CustomerBot` — high-level coordinator/state machine (approach, queue slot, waiting for table, moving to table, eating, leaving) that delegates to `CustomerOrderView` (TMP order count and `NO SEAT!` presentation), `CustomerNavigation` (NavMeshAgent adapter: destinations, stop/resume, arrival checks, readiness guards), and `CustomerDiningController` (table reservation, eating timer, leftover creation, seat release). `CustomerSpawner` — configurable timed coroutine spawning customers with random orders. |
...
| `Assets/Engineering/Prefabs/CustomerBot.prefab` | Customer-bot prefab — capsule visual, NavMeshAgent, CapsuleCollider, `CustomerBot` coordinator with `CustomerOrderView`, `CustomerNavigation`, and `CustomerDiningController` components. |
...
- `CustomerBot` is the high-level coordinator/state machine. It owns approach waypoint traversal, queue-slot assignment, and the `WaitingForTable`/`MovingToTable`/`Eating`/`Leaving` transitions, delegating UI to `CustomerOrderView`, movement to `CustomerNavigation`, and table/dining lifecycle to `CustomerDiningController`. When `TransitionToDining` cannot reserve a table (no free seat or insufficient leftover capacity), the customer enters `WaitingForTable` and `CustomerOrderView` shows the Inspector-configurable `noSeatMessage` (default `NO SEAT!`). The message is hidden when `RetryReserveTable` succeeds and the customer moves to the table. Normal count display is preserved for incomplete orders.
...
- The project source currently declares 296 EditMode and 181 PlayMode tests, including camera-follow and `SCameraSettings` coverage. Run the affected suite after gameplay changes; test counts alone do not prove they passed.
