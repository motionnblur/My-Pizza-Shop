...
| `Assets/Engineering/Scripts/Mono/Actors/CustomerQueue/` | `CustomerBot` — NavMeshAgent-driven NavMesh/Transform adapter with approach-waypoint traversal and queue-slot movement; owns the overhead `orderText` UI, which shows `NO SEAT!` (`noSeatMessage`, Inspector-configurable) while a completed customer waits for a table. `CustomerSpawner` — configurable timed coroutine spawning customers with random orders. |
