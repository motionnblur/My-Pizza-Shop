...
| Pizza serving | `ServeStation` manages an ordered `List<CustomerBot>` queue. `RegisterCustomer` assigns queue-slot transforms immediately. `TryServeFrontCustomer` transfers pizzas only from the front customer, awards money for delivered pizzas, raises `PizzaServed`, and removes/reassigns slots on completion. Obsolete pizza-visual pool and `TryServeAll` were removed. | Confirmed | `SServeStation.cs`, `ServeStation.cs`, `ServePlate.cs`, `PlayerPizzaInventory.cs`, `EconomyManager.cs`, `CustomerBot.cs` |
