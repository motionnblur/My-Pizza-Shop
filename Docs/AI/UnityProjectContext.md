...
| Customer queue | `CustomerBot` is the NavMesh/Transform adapter: traverses approach waypoints then moves to its assigned queue slot via `NavMeshAgent`. `CustomerBot` owns the overhead `orderText` UI: it shows the remaining pizza count while ordering, and when a completed order cannot transition to dining (no free seat or insufficient leftover capacity) it enters `WaitingForTable` and shows the Inspector-configured `noSeatMessage` (`NO SEAT!` by default); the message is hidden again when `RetryReserveTable` succeeds. `CustomerSpawner` runs a timed coroutine, randomizing orders within configured min/max and respecting station capacity. | Confirmed | `CustomerBot.cs`, `CustomerSpawner.cs`, `SServeStation.cs` |
...
| Leftover waste | `TableWasteModel` is the authoritative owner of leftover count. `Table` delegates leftover state to `TableWasteModel` and optional `TableWasteVisuals`; raises `LeftoverStateChanged` on `TryRemoveLeftovers`/`ClearLeftovers`. `TableManager` exposes `AddLeftoversToTable(tableIndex, pizzaCount)`, lazily subscribes to table change events before table operations, raises `SeatReleased` on seat release, and re-raises `LeftoversRemoved`. When `CustomerBot` transitions from Eating to Leaving, it adds the order's initial pizza count before releasing its seat. `TableWasteTrigger` transfers leftovers into `PlayerWasteInventory` only while the player holds no pizzas. `TrashStation` separately pools and animates waste disposal, then raises `WasteDisposed`. | Confirmed | `WasteInventoryModel.cs`, `PlayerWasteInventory.cs`, `TableWasteTrigger.cs`, `TableWasteModel.cs`, `TableWasteVisuals.cs`, `Table.cs`, `TableManager.cs`, `CustomerBot.cs`, `TrashStation.cs` |
...
- **PlayMode tests:** 176 `[UnityTest]` declarations in `Assets/Engineering/Tests/PlayMode/`; they cover economy, production, serving/queue flow, tables and leftover collection, pizza and waste disposal animations/events/pool cleanup, UI, and real-physics Player trigger regressions. Execution was not performed in this audit.
