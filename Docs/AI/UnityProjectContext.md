...
| Camera | MainScene has one URP `Main Camera`, orthographic with size 5; a separate `CameraManager` child of `Managers` follows the Player while controlling the camera through an explicit reference. On startup the camera snaps to `target.position + settings.FollowOffset` (default `-7, 10, -7`) while preserving the initial scene-authored rotation. It applies per-axis `SmoothDamp` damping (`xDamping`, `yDamping`, `zDamping`) with an optional `maxFollowSpeed` limit. Damping values and follow offset are stored in the assigned `SCameraSettings` ScriptableObject asset. Cinemachine is not present in the manifest or first-party code | Confirmed | `Assets/Scenes/MainScene.unity`, `Assets/Engineering/Scripts/Mono/Managers/CameraManager.cs`, `Assets/Engineering/ScriptableObjects/SCameraSettings.cs`, `Assets/Engineering/ScriptableObjects/SCameraSettings.asset`, `Packages/manifest.json` |
...
| Tests | Unity Test Framework 1.6.0 is installed; current source declares 296 EditMode `[Test]` methods and 181 PlayMode `[UnityTest]` methods covering core gameplay, camera follow and `SCameraSettings` behavior, UI, economy, money-animation pooling, pizza and waste inventories, grill production, serving station with customer queue, customer queue domain models, purchase progress domain models, table flow, and pizza/waste trash-station pooling and cleanup | Confirmed declaration counts; execution not performed in this audit | `Packages/manifest.json`, `Assets/Engineering/Tests/` |
...
| `Assets/Engineering/Scripts/Mono/Actors/CustomerQueue/` | `CustomerBot` coordinator/state machine plus extracted `CustomerOrderView` (TMP order count and `NO SEAT!` presentation), `CustomerNavigation` (NavMeshAgent adapter), and `CustomerDiningController` (table reservation, eating, leftovers, seat release); timed customer spawner | Confirmed | `CustomerBot.cs`, `CustomerOrderView.cs`, `CustomerNavigation.cs`, `CustomerDiningController.cs`, `CustomerSpawner.cs` |
| `Assets/Engineering/Scripts/Mono/Actors/TrashStation/` | Separate player pizza and waste disposal flows with pooled DoTween animations | Confirmed | `TrashStation.cs`, `TrashPlate.cs` |
...
| Customer queue | `CustomerBot` is the high-level coordinator/state machine: it owns approach-waypoint traversal, queue-slot assignment, and the `WaitingForTable`/`MovingToTable`/`Eating`/`Leaving` transitions, delegating UI to `CustomerOrderView`, movement to `CustomerNavigation` (destinations, stop/resume, arrival checks, NavMesh readiness guards), and table/dining lifecycle to `CustomerDiningController` (table reservation, eating timer, leftover creation, seat release). `CustomerOrderView` shows the remaining pizza count while ordering, and when a completed order cannot transition to dining (no free seat or insufficient leftover capacity) it shows the Inspector-configured `noSeatMessage` (`NO SEAT!` by default); the message is hidden again when `RetryReserveTable` succeeds. `CustomerSpawner` runs a timed coroutine, randomizing orders within configured min/max and respecting station capacity. | Confirmed | `CustomerBot.cs`, `CustomerOrderView.cs`, `CustomerNavigation.cs`, `CustomerDiningController.cs`, `CustomerSpawner.cs`, `SServeStation.cs` |
| Pizza and waste trash | `TrashStation` has separate pooled disposal flows for pizzas and waste. It removes each stack, animates its visuals to `TrashTarget` with staggered DoTween (`DOMove` + `DOScale(0)`), releases them back to the matching pool, and raises `PizzaTrashed` or `WasteDisposed`. | Confirmed | `STrashStation.cs`, `TrashStation.cs`, `TrashPlate.cs`, `PlayerPizzaInventory.cs`, `PlayerWasteInventory.cs` |
| Leftover waste | `TableWasteModel` is the authoritative owner of leftover count. `Table` delegates leftover state to `TableWasteModel` and optional `TableWasteVisuals`; raises `LeftoverStateChanged` on `TryRemoveLeftovers`/`ClearLeftovers`. `TableManager` exposes `AddLeftoversToTable(tableIndex, pizzaCount)`, lazily subscribes to table change events before table operations, raises `SeatReleased` on seat release, and re-raises `LeftoversRemoved`. When `CustomerDiningController` finishes eating it adds the order's initial pizza count before releasing the seat. `TableWasteTrigger` transfers leftovers into `PlayerWasteInventory` only while the player holds no pizzas. `TrashStation` separately pools and animates waste disposal, then raises `WasteDisposed`. | Confirmed | `WasteInventoryModel.cs`, `PlayerWasteInventory.cs`, `TableWasteTrigger.cs`, `TableWasteModel.cs`, `TableWasteVisuals.cs`, `Table.cs`, `TableManager.cs`, `CustomerDiningController.cs`, `TrashStation.cs` |
...
| Gameplay feedback | `EconomyManager` raises `GroundMoneyCollected` and `BuyingAreaPurchased`; `ServeStation` raises `PizzaServed`; `TrashStation` raises `PizzaTrashed` and `WasteDisposed`; `SoundManager` subscribes and maps assigned events to `SSound` clips through `SVoidEventChannel` assets; `AnimationManager` and `EconomyManager` share the `MoneyAnimationRequested.asset` channel in MainScene | Confirmed | `EconomyManager.cs`, `ServeStation.cs`, `TrashStation.cs`, `SoundManager.cs`, `SMoneyAnimationEventChannel.cs`, `MoneyAnimationRequest.cs` |
...
- **EditMode tests:** 296 `[Test]` declarations in `Assets/Engineering/Tests/Editor/`; they cover wallet, trigger relays, movement, camera follow and `SCameraSettings` tuning, event channels, pizza and waste inventory capacity, station/domain rules, table and leftover rules, scene/prefab configuration, and the Player prefab hierarchy contract. Execution was not performed in this audit.
- **PlayMode tests:** 181 `[UnityTest]` declarations in `Assets/Engineering/Tests/PlayMode/`; they cover economy, production, serving/queue flow, tables and leftover collection, pizza and waste disposal animations/events/pool cleanup, UI, and real-physics Player trigger regressions. Execution was not performed in this audit.
...
- The current camera is an orthographic scene camera with per-axis SmoothDamp Player follow; the follow offset is driven by `SCameraSettings.FollowOffset` (default `-7, 10, -7`) and the camera snaps to `target.position + FollowOffset` on startup, preserving the scene-authored initial rotation. Look input is already relayed but camera rotation and zoom behavior are not implemented.
...
- `Assets/Engineering/Scripts/Mono/Actors/CustomerQueue/CustomerOrderView.cs`
- `Assets/Engineering/Scripts/Mono/Actors/CustomerQueue/CustomerNavigation.cs`
- `Assets/Engineering/Scripts/Mono/Actors/CustomerQueue/CustomerDiningController.cs`
...
- `Assets/Engineering/Tests/Editor/CustomerBotPrefabContractTests.cs`
...
- `Assets/Engineering/Tests/PlayMode/TablePlayModeTests.cs`
